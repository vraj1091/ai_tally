...
class TallyDataService:
    ...
    def get_ledgers_for_company(
        ...
        if self.connected and self.tally_connector:
            try:
                # Fetch ALL ledgers safely with very reduced limit
                ledgers = self.tally_connector.get_ledgers(company_name, limit=500, fetch_all=True)
    ...
    def _calculate_summary_from_backup_data(self, cached_data: Dict) -> Dict:
        """Calculate summary from backup data (same logic as backup_routes.py)"""
        try:
