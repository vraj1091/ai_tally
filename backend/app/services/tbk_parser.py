...
class TallyBackupParser:
    ...
    def parse_xml_data(self, xml_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if file_size_mb > 20:
                logger.warning(f"Large file detected ({file_size_mb:.2f} MB) - parsing may take 2-5 minutes and use significant memory")
                logger.warning(f"For faster processing, consider: 1) Compress to .zip, 2) Export smaller date range, 3) Split into quarters")
    ...
    def _get_amount(self, element: ET.Element, tag: str) -> float:
        ...
        try:
            # Remove currency symbols, commas, Dr/Cr, and whitespace
            # Handle negative numbers in various formats: -100, 100 Dr, (100)
            cleaned = text.replace("₹", "").replace(",", "").replace("Rs", "").replace("rs", "").strip()
            
            is_negative = False
            if "Dr" in text or "dr" in text:
                is_negative = True # Debit is usually positive for assets/expenses, but Tally context varies. 
                                   # We usually want absolute magnitude here.
            if cleaned.startswith("-") or "(" in cleaned:
                is_negative = True
            
            cleaned = cleaned.replace("Dr", "").replace("Cr", "").replace("dr", "").replace("cr", "").replace("(", "").replace(")", "").strip()
            
            if cleaned:
                amount = float(cleaned)
                return abs(amount) # Always return absolute, let caller decide sign based on context
            return 0.0
    ...
    def _parse_ledger(self, element: ET.Element) -> Optional[Dict]:
        ...
        try:
            ...
            # Try all closing balance fields
            for field in balance_fields:
                val = self._get_amount(element, field)
                if val > closing_bal:
                    closing_bal = val
                if val > balance:
                    balance = val
            
            # Try all opening balance fields
            ...
            for field in balance_fields:
                ...
                if nested_elem is not None:
                    val = self._get_amount(nested_elem, ".")
                    if val > closing_bal:
                        closing_bal = val
                    if val > balance:
                        balance = val
            ...
            for child in element.iter():
                ...
                if any(keyword in tag_upper for keyword in ['AMOUNT', 'BALANCE', 'BAL']):
                    try:
                        # Try text content
                        if child.text:
                            cleaned = child.text.replace('₹', '').replace(',', '').replace('Dr', '').replace('Cr', '').replace('Rs', '').strip()
                            if cleaned:
                        ...
                        for attr in child.attrib:
                            if 'amount' in attr.lower() or 'balance' in attr.lower() or 'bal' in attr.lower():
                                try:
                                    val = abs(float(child.attrib[attr]))
                                    if val > closing_bal:
                                        closing_bal = val
                                    if val > balance:
                                        balance = val
            ...
            if closing_bal == 0 and opening_bal != 0:
                closing_bal = opening_bal
            if balance == 0:
                balance = closing_bal or opening_bal
            
            # Log significant balances for debugging
            if closing_bal > 1000 or opening_bal > 1000:
