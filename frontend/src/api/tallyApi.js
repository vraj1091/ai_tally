...
export const tallyApi = {
  ...
  // Get detailed analytics for a company - used by Analytics page
  getDetailedAnalytics: async (companyName, dataSource = 'live') => {
    try {
      console.log(`📊 Fetching detailed analytics for: ${companyName} (source: ${dataSource})`);

      // Determine the endpoint based on data source
      let endpoint = `/analytics/${encodeURIComponent(companyName)}?source=${dataSource}`;

      // Also try backup route if in backup mode
      if (dataSource === 'backup') {
        try {
          ...
          console.warn('Analytics endpoint failed, trying backup data...');
        }

        // Fallback: get backup data and calculate analytics
        try {
          const backupData = await tallyApi.getBackupData(companyName);
          if (backupData && backupData.ledgers) {
            const ledgers = backupData.ledgers || [];
            let totalRevenue = 0;
            let totalExpenses = 0;

            ledgers.forEach(ledger => {
              const balance = parseFloat(ledger.closing_balance) || 0;
              const parent = (ledger.parent || '').toLowerCase();

              if (parent.includes('sales') || parent.includes('income') || parent.includes('revenue')) {
                totalRevenue += Math.abs(balance);
              } else if (parent.includes('expense') || parent.includes('purchase')) {
          ...
          console.error('Backup data fetch failed:', e);
        }
      }

      // For live/bridge mode, use the analytics endpoint
      try {
        const response = await apiClient.get(endpoint, { timeout: 60000 });
        const data = response.data?.data || response.data;
        ...
        console.warn('Analytics API failed, trying CEO dashboard as fallback...');

        // Fallback: Use CEO dashboard data
        try {
          const ceoData = await tallyApi.getCEODashboardData(companyName, dataSource);
          const execSummary = ceoData?.data?.executive_summary || ceoData?.executive_summary || ceoData || {};
          return {
            data: {
              total_revenue: execSummary.total_revenue || 0,
              total_expenses: execSummary.total_expenses || execSummary.total_expense || 0,
              net_profit: execSummary.net_profit || 0,
              source: dataSource
            }
          };
        } catch (ceoError) {
          console.error('CEO dashboard fallback also failed:', ceoError);
          return { data: null };
        }
      }
    } catch (error) {
