...
const SalesDashboard = ({ dataSource = 'live' }) => {
  ...
  const loadSalesData = async () => {
    setLoading(true);
    try {
      const response = await apiClient.get(`/dashboards/sales/${encodeURIComponent(selectedCompany)}?source=${dataSource}`);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white rounded-xl shadow-sm p-6 border-l-4 border-purple-500">
          ...
          <p className="text-sm text-gray-600 mt-1">{formatCurrency(Math.abs(topCustomers[0]?.closing_balance || 0))}</p>
        </div>
        <div className="bg-white rounded-xl shadow-sm p-6 border-l-4 border-blue-500">
          ...
          <p className="text-sm text-gray-600 mt-1">{formatCurrency(Math.abs(topProducts[0]?.closing_balance || 0))}</p>
        </div>
