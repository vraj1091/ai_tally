...
export default function AnalyticsPage() {
  ...
  const loadAnalytics = async () => {
    ...
    try {
      ...
      // Extract data properly - handle various response structures
      let data = null;
      if (res?.data?.data) {
        data = res.data.data;
      } else if (res?.data) {
      ...
      // If data has executive_summary, extract values from there
      if (data?.executive_summary) {
        const exec = data.executive_summary;
        data = {
          ...data,
          total_revenue: data.total_revenue || exec.total_revenue || 0,
          total_expenses: data.total_expenses || exec.total_expenses || exec.total_expense || 0,
          net_profit: data.net_profit || exec.net_profit || 0
        };
      }

      console.log('[AnalyticsPage] Loaded data:', data);
